...
| **factions.h** | Faction enums (DAWNGUARD, IRONBONE, ASHCLAW, VEILSTORM); UnitType (9 values: 6 playable roles + UNIT_ARMORED/WARDED/FLYER, enemy-only — see `PLAYABLE_UNIT_TYPE_COUNT`); `DamageType` (MELEE/RANGED/MAGIC) for the resistance system |
| **units_data.h** | UnitStats struct (HP, damage, cost, etc, plus `damage_type`/`immune_to_mask` for the resistance system) and `unit_stats_blocks_damage()` |
| **units_data.c** | Full stat table for 36 units (4 factions x 9 types: 24 playable + 12 enemy-only elite variants), each playable one with its own active ability |
...
| **entities.c** | Towers, runners, projectiles. Combat, movement, the 24 unique special abilities, damage-type resistance checks (`unit_stats_blocks_damage()`, a shared `resolve_source_tower()` lookup), target acquisition that prefers a non-immune enemy over a fully-resisted one, rendering in color-grouped passes (health bars/dots/debug) |
...
**36 animated unit sheets** (4 factions x 9 types: 6 playable roles + 3
enemy-only elite variants — see [Enemy Resistances](README.md#enemy-resistances)),
256x32px each (8 frames of 32x32: idle + 4 walk + 3 attack):
```
...
dawnguard_armored.png    ironbone_armored.png
dawnguard_warded.png     ironbone_warded.png
dawnguard_flyer.png      ironbone_flyer.png
...
ashclaw_armored.png      veilstorm_armored.png
ashclaw_warded.png       veilstorm_warded.png
ashclaw_flyer.png        veilstorm_flyer.png
```
...
**1 reference sprite sheet** (overview of all 36 units in idle pose + projectiles):
```
sprite_sheet.png
```
...
Generates the 36 animated unit sheets (24 playable + 12 enemy-only elite
variants: `draw_armored`/`draw_warded`/`draw_flyer`) + 4 projectiles + the
reference sheet. Each `draw_<type>(palette, faction, frame)` function draws
the base silhouette and shifts the legs/weapon (or wings, for the Flyer)
based on the frame (`anim_offsets()`)
...
Total sprites:             47 generated PNGs (40 units/projectiles + 7 terrain tiles)
                            + 4 reference maps + 1 reference sheet
Factions:                  4
Unit types per faction:    9 (6 playable, each with a unique active ability,
                            + 3 enemy-only elite variants with damage-type
                            immunities — see Enemy Resistances in README.md)
